...
> type ALH    = [AState] -- agent life history is a list of agent states

**** 1.2 Group level **************************************

> type GState = V.Vector AState -- group state is a list of agent states
> type GLH = [GState] -- group life history is a list of group states
...
- aState: current state of the ALH that the formula is to be checked upon
...
>                                return r2
>                              else
>                                return $ r2 `aAnd` (r1 `aOr` (True, ARelease f1 f2))
>       (AFinally f)     -> do r <- checkA f e
>                              if l then 
>                                return r
>                              else 
>                                return $ r `aOr` (True, AFinally f)
>       (AGlobally f)    -> do r <- checkA f e
>                              if l then 
>                                return r
>                              else 
>                                return $ r `aAnd` (True, AGlobally f)
...
- rState: current state of the GLH that the formula is to be checked upon
...
>                                 return r2 
>                               else
>                                 return $ r2 `gOr` (r1 `gAnd` (True, GObl (GUntil f1 f2) a))
...
>                                 return r
>                               else 
>                                 return $ r `gOr` (True, GObl (GFinally f) a)
>        (GGlobally f)    -> do r <- checkGState' f e
>                               if l then 
>                                 return r
>                               else 
>                                 return $ r `gAnd` (True, GObl (GGlobally f) a)
...
For agent life histories:

> ppALH :: ALH -> String
> ppALH []     = ""
> ppALH (x:xs) = show x ++ " -> " ++ ppALH xs

> ppGLH :: GLH -> String
> ppGLH []     = ""
> ppGLH (x:xs) = show x ++ " -> " ++ ppGLH xs
